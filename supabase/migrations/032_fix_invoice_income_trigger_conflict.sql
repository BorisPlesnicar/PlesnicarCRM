-- Fix: ON CONFLICT muss zur (partial) Unique-Index-Inferenz passen

-- Ersetze den alten Unique-Index (falls vorhanden)
drop index if exists public.transactions_unique_invoice_income_idx;

-- Eindeutigkeit: pro invoice_id + type genau eine Auto-Buchung (invoice_id kann NULL sein)
create unique index if not exists transactions_unique_invoice_id_type_idx
  on public.transactions(invoice_id, type)
